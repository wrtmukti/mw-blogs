...
<body>
  <!--Navbar -->
  <nav class="mb-1 navbar navbar-expand-lg navbar-light white fixed-top mb-5">
    <!-- logo -->
    ...
    <div class="collapse navbar-collapse" id="navbarSupportedContent-555">
      <ul class="navbar-nav mr-auto">
        ...
        <li class="nav-item">
          <?php
          if ($this->session->userdata('email')) { ?>
            <a class="btn btn-success my-2 my-sm-0  rounded-pill" href="<?= base_url() ?>article/create">Buat Artikel</a>
          <?php } ?>
        </li>
      </ul>
      <!-- auth -->
      <div>
        ...
        if (!$this->session->userdata('email')) { ?>
          <div class="row">
            ...
            <a class="nav-link btn btn-primary  my-2 my-sm-0  rounded-pill " href="<?= base_url('auth/registration') ?>">Daftar</a>
          </div>
        <?php } else {  ?>
          <div class="mx-3">
            <div class="row nav-item dropdown">
              <img src="<?= base_url('assets/img/profile/') . $users['image']; ?>" class="rounded-circle z-depth-0" alt="avatar image" height="45" width="45">
